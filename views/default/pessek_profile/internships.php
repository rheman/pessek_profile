...
$internships_Image = elgg_get_site_url() . "mod/pessek_profile/img/internships.png";

$user = get_user($user_guid);

$internships_guid = $user->internships;

$month_selected = array('January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December');

echo '<div class="gcconnex-internships">';

    echo '<div class="gcconnex-profile-internships-display internships_readmore">';

    echo '<table width="100%">';

    if ($user->canEdit() && ($internships_guid == NULL || empty($internships_guid))) {
        echo elgg_echo('gcconnex_profile:internship:empty');
    }
    else {
            ...
            if ($entry = get_entity($guid)) {
                ...
                echo '<tr>
                    <td width="100%">';
                            ...
                            echo '<div class="gcconnex-profile-internships-display gcconnex-internships-' . $entry->guid . '">';
                ...
                /*
                    ...
                    echo '<td class="button-delete-update-profile"><p data-placement="top" data-toggle="tooltip" title="'. elgg_echo('gcconnex_profile:internship:delete') .'">
                                <span class="btn btn-danger btn-xs glyphicon glyphicon-trash" onclick="entryDeletion(\''.$user_guid.'\',\''.$guid.'\',\''.elgg_echo('gcconnex_profile:internship:delete').'\',\''.$section.'\',\''.$deletionmessage.'\',\''.elgg_echo('gcconnex_profile:pessek:yes').'\',\''.elgg_echo('gcconnex_profile:pessek:cancel').'\')"></span></p></td>';
