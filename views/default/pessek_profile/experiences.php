...
$internships_Image = elgg_get_site_url() . "mod/pessek_profile/img/experiences.png";

$user = get_user($user_guid);

$internships_guid = $user->experiences;

$month_selected = array('January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December');

echo '<div class="gcconnex-experiences">';

    echo '<div class="gcconnex-profile-experiences-display experiences_readmore">';

    echo '<table width="100%">';

    if ($user->canEdit() && ($internships_guid == NULL || empty($internships_guid))) {
        echo elgg_echo('gcconnex_profile:experiences:empty');
    }
    else {
            ...
            if ($entry = get_entity($guid)) {
                ...
                echo '<tr>
                    <td width="100%">';
                            ...
                            echo '<div class="gcconnex-profile-experiences-display gcconnex-experiences-' . $entry->guid . '">';
                            ...
                            echo'<p class="paraph-justify"><img src="' . $internships_Image .'" style="float: left; margin-left: 10px; margin-right: 10px;"><strong>' . $entry->jobtitle .'</strong><br>' . $entry->companyname . ' ' . $internships_date . ' , ' . $work_duration . '' . $country . '' . $the_link .'' . nl2br($thedescription) . '</p>';
                ...
                /*
                    ...
                    echo '<td class="button-delete-update-profile"><p data-placement="top" data-toggle="tooltip" title="'. elgg_echo('gcconnex_profile:experience:delete') .'">
                                <span class="btn btn-danger btn-xs glyphicon glyphicon-trash" onclick="entryDeletion(\''.$user_guid.'\',\''.$guid.'\',\''.elgg_echo('gcconnex_profile:experience:delete').'\',\''.$section.'\',\''.$deletionmessage.'\',\''.elgg_echo('gcconnex_profile:pessek:yes').'\',\''.elgg_echo('gcconnex_profile:pessek:cancel').'\')"></span></p></td>';
